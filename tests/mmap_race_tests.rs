...
fn test_race_concurrent_offset_allocation() {
    ...
    let writer = Arc::new(MmapCsvWriter::new(&path, 100 * 1024 * 1024).unwrap());
...
fn test_race_rapid_small_writes() {
    ...
    let writer = Arc::new(MmapCsvWriter::new(&path, 20 * 1024 * 1024).unwrap());
        ...
        .map(|thread_id| {
            ...
            thread::spawn(move || {
                ...
                while !stop_flag.load(Ordering::Relaxed) {
                    thread_writer.write_lines(&[thread_id.to_string()]).unwrap();
                    count += 1;
        ...
        thread::spawn(move || {
            ...
            for batch in 0..100 {
                let lines: Vec<String> =
                    (0..100).map(|i| format!("batch,{}_{}", batch, i)).collect();
