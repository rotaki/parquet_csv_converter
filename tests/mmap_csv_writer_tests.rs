...
use parquet_csv_converter::mmap_csv_writer::{estimate_csv_size, MmapCsvWriter};
...
fn test_stress_many_small_writes() {
    ...
    let writer = Arc::new(MmapCsvWriter::new(&path, 50 * 1024 * 1024).unwrap());
...
fn test_large_batch_writes() {
    ...
    let large_batch: Vec<String> = (0..100000).map(|i| format!("{},{}", i, i * 2)).collect();
...
fn test_buffered_writes() {
    ...
    thread_writer
        .write_lines_buffered(lines, 1024 * 1024)
        .unwrap();
...
fn test_file_size_exceeded() {
    ...
    let writer = MmapCsvWriter::new(&path, 1024).unwrap(); // Only 1KB
    writer.write_header("id,data").unwrap();

    let thread_writer = writer.get_thread_writer();

    // Try to write more than available space
    // Header takes 9 bytes ("id,data\n"), leaving 1015 bytes
    // We'll try to write 1020 bytes (1019 chars + newline)
    let large_data = vec!["x".repeat(1019)];
...
fn test_empty_lines() {
    ...
    thread_writer.write_lines(&[]).unwrap();

    // Write some data
    thread_writer.write_lines(&["a,b".to_string()]).unwrap();
...
fn test_concurrent_readers_during_write() {
    ...
    let writer1 = {
        ...
        thread::spawn(move || {
            ...
            for i in 0..1000 {
                thread_writer
                    .write_lines(&[format!("w1_{},value_{}", i, i)])
                    .unwrap();
            }
        })
    };
    ...
    let writer2 = {
        ...
        thread::spawn(move || {
            ...
            for i in 0..1000 {
                thread_writer
                    .write_lines(&[format!("w2_{},value_{}", i, i)])
                    .unwrap();
            }
        })
    };
